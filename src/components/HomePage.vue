<template>
    <div class="homeContainer">
        <div class="homeContent">
            ...
            <div class="contentItem">
                지난 1개월간의 거래내역을 확인해보세요
            </div>
            <h1 class="contentItem">₩{{ totalMoney.toLocaleString() }}</h1>
            <div class="amountBoxContainer">
                <div
                    class="amountBox"
                    :class="{ greenText: totalAmountPlus > 0 }"
                >
                    <div style="color: black">들어온돈</div>
                    <span>+{{ totalAmountPlus.toLocaleString() }}</span>
                </div>
                <div
                    class="amountBox"
                    :class="{ redText: totalAmountMinus < 0 }"
                >
                    <div style="color: black">나간 돈</div>
                    <span>{{ totalAmountMinus.toLocaleString() }}</span>
                </div>
            </div>
        </div>
    </div>
